...
function onTristateImageClick(imageId, standAloneMode,onChangeFunction) {
	return function() {
		var fieldAndContainerIds = getFieldAndContainerIds(imageId);
		var triStateBoxField = document.getElementById(fieldAndContainerIds[0]);
		...
		if (onChangeFunction !== undefined) {
			onChangeFunction(nextState);
		}
	}
}

function triChangeState(imageId) {
	//var triStateBoxField =  $('#tristateBox2');
	if (document.getElementById('tristateBox2.Field') === null) return;
	var fieldAndContainerIds = getFieldAndContainerIds('tristateBox2' + '.Img');
	
		
	if (fieldAndContainerIds != null ) {
		var triStateBoxField = document.getElementById(fieldAndContainerIds[0]);

		triStateBoxField.value = imageId;
		var imageSrc = mouseOverOutOfImage('tristateBox2' + '.Img', true);
		replaceImage('tristateBox2' + '.Img', imageSrc);
...
function createTriStateImageNode(triStateBoxNode, imageNodeId, standAloneMode,onChangeFunction) {
	...
	if (triStateBoxNode.addEventListener) {
		...
		triStateBoxNode.addEventListener('click', onTristateImageClick(imageNode.id, standAloneMode,onChangeFunction), false);
...
function initTriStateCheckBox(triStateBoxId, containerOrStateFieldId, standAloneMode,onChangeFunction) {
	...
	createTriStateImageNode(triStateBoxNode, imageNodeId, standAloneMode,onChangeFunction);
